...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> listFilesAndDirs() async {
    final dir = Directory("C:/Bhargav");
    files = dir.listSync();
    for (var element in files) {
      try{
        final type = await FileSystemEntity.type(element.path);
        print("$type \n");
      }
      catch(exception){
  ...
  Widget _buildDirectory(BuildContext context, AsyncSnapshot<Directory?> snapshot) {
    ...
    if (snapshot.connectionState == ConnectionState.done) {
      if (snapshot.hasError) {
        ...
        listFilesAndDirs();
        text = Text('path: ${snapshot.data!.path} , ${files.toString()}');
      } else {
